...
<p class="chg">예비창업자로서 이미 동작하는 K-Rosetta 시제품을 상용 서비스로 키워 창업합니다. 먼저 착수 후 6개월 안에 법인을 설립하고, 예비창업패키지·청년창업사관학교 같은 정부 창업지원사업에 지원해 초기 개발·운영 자금을 확보합니다. 이미 동작하는 시제품이 있어 선정에 유리합니다. 이와 함께 정확성이 반드시 필요한 공공기관(국립박물관·한국관광공사·재외문화원)에 무상 또는 저가로 파일럿을 제공해 첫 도입 실적과 사용 확인서를 확보하고, 이 실적을 이후 영업과 투자 유치의 근거로 삼습니다.</p>
<p class="chg">1년 차에는 지금의 데모를 상용 서비스로 다듬습니다. 클라우드에 배포하고 이용자 계정과 과금 체계를 붙이며, API를 정리하고 용어사전을 500개 이상으로 확대합니다. 그다음 웹툰·자막·번역업체를 대상으로 용어 검수 API와 구독 서비스를 유료로 출시해 첫 매출을 만들고, 문화용어 다국어 데이터 라이선스로 수익원을 넓힙니다. 이렇게 공공 도입 실적과 초기 매출이 쌓이면 시드 투자나 정부 R&D를 유치하고, 글로벌 진출 지원사업과 연계해 저자원 언어권 시장으로 확장합니다.</p>
<p><b>경쟁 우위</b></p>
<ul>
