...
<p>K-Rosetta는 한국국제교류재단과 한국국제협력단이 공개한 공공데이터의 표준 용어를 활용하여, 문화용어를 정해진 표기에 맞게 번역하는 서비스입니다. 번역은 단어를 바꾸는 데 그치지 않고 그 말에 담긴 맥락을 함께 옮기는 일입니다. 문화를 전달하는 가장 기본이 되는 것이 언어이므로, 문화용어를 정확한 표기와 뜻으로 옮길 때 비로소 한국의 문화와 고유한 가치가 왜곡 없이 전해질 수 있습니다. K-Rosetta는 이 지점을 목표로, 입력 문장에서 문화용어를 자동으로 찾아 공공데이터에 정리된 공인 표기를 적용하고 번역의 근거가 되는 자료를 함께 보여 줍니다. 서비스가 사용하는 자료는 모두 공공데이터와 「Koreana」 관례에서 나온 것으로, 문화·전문 용어 1,211개(한국음식정보 691개, 개발협력 용어 393개, 문화용어 93개, 해외대학 기관명 34개)와 이를 여러 언어로 옮긴 표준 대역 1,643개, 그리고 번역의 근거로 삼는 문서 1,542건(「Koreana」 기사 480건 포함)으로 이루어져 있습니다.</p>
<p>이 자료를 바탕으로 문화용어 64개를 4개 언어에서 비교 측정했습니다. 일반 번역기가 한국의 공인 표준 표기를 따르는 비율은 평균 23.4%였고, 저자원 언어인 베트남어와 인도네시아어에서는 12.5%까지 낮아졌습니다. K-Rosetta는 공공데이터의 표준 표기를 그대로 적용하기 때문에 같은 기준에서 100%를 기록했습니다. 여기서 중요한 것은 K-Rosetta의 100%보다도, 일반 번역기가 공공 표준을 23% 정도밖에 따르지 못한다는 점입니다.</p>
<p>본 서비스는 실제로 동작하는 형태로 개발을 마쳤으며, 발표 심사에서 직접 시연할 수 있습니다.</p>
...
<h2>8. 시장성 및 사업화 전략</h2>
<h3>가. 제품의 시장 정의 — 범용 번역기와 경쟁하지 않는다</h3>
<p>K-Rosetta는 구글 번역·파파고·DeepL 같은 범용 번역기와 정면으로 경쟁하지 않습니다. 범용 번역의 품질·속도·언어 커버리지는 이미 대형 플랫폼의 영역이며, 후발 주자가 그 축에서 이기기는 어렵습니다. K-Rosetta의 자리는 다릅니다. 한국어 문화 코퍼스와 공공데이터 표준을 기준으로 <b>문화·전통·역사 용어를 정확히 옮기고 검수하는 특화 계층</b>이며, 범용 번역 위에 얹혀 공인 표기를 강제하는 <b>보완재</b>입니다. 즉 '더 좋은 번역기'가 아니라 '정확성이 반드시 필요한 곳을 위한 검수·표준화 엔진'으로 시장을 정의합니다.</p>

<h3>나. 목표 시장과 진입 순서 — 정확성이 '의무'인 곳부터</h3>
<table>
<tr><th style="width:12%">세그먼트</th><th style="width:24%">대상 고객</th><th>해결하는 문제 (왜 급한가)</th><th style="width:20%">수익 모델</th></tr>
...
<tr><td><b>B2B</b><br>(확장)</td><td>웹툰·웹소설 플랫폼, OTT 자막 제작 벤더, 번역(LSP)업체, 관광·출판</td><td>대량 현지화에서 용어 일관성·품질 검수가 인력 병목</td><td>API 종량제·팀 구독·데이터 라이선스</td></tr>
<tr><td><b>B2C</b><br>(저변)</td><td>한류 팬·한국어 학습자·개별 여행자</td><td>문화용어를 정확히 알고 싶은 수요</td><td>무료 — 데이터·브랜드·상위 유입 퍼널</td></tr>
</table>
...
<h3>다. 수익 모델 (다층 구조)</h3>
<ul>
<li><b>라이선스·구독</b> — 공공기관·기업에 표준 용어 검수/번역을 연간 구독 또는 구축형으로 도입</li>
<li><b>API 종량제</b> — 개발사가 자사 번역·현지화 파이프라인에 용어 검수 API를 요청량 기준으로 사용</li>
<li><b>데이터 라이선스</b> — 문화용어 다국어 대역·평가셋을 번역·LLM 업계에 제공</li>
<li><b>B2C 무료</b> — 직접 매출원이 아니라 데이터 축적·브랜드·상위 상품 유입의 통로</li>
</ul>
<p class="small">※ 정량 시장 규모는 발표 단계에서 KOCCA 콘텐츠산업통계·OTT 현지화 시장 자료로 보강하며, 본 기획서에는 검증되지 않은 추정치를 넣지 않았습니다.</p>
...
<h2>10. 제품 및 서비스의 사업(창업) 계획</h2>
<p><span class="small">□ 예비창업자 / □ 기창업자 (해당란 표시)</span></p>
<h3>가. 사업화 로드맵 (현실적 순서)</h3>
<table>
<tr><th style="width:22%">단계</th><th>내용</th></tr>
<tr><td>1단계<br>(~2026.4분기)<br>레퍼런스 확보</td><td>박물관·관광공사 등 공공기관 1~2곳 파일럿(무상/저가)으로 첫 도입 실적을 만들고, 동시에 B2C 무료 서비스를 공개합니다. 용어사전을 500개 이상으로 확대하고 한국국제교류재단에 데이터 활용 협력을 제안합니다.</td></tr>
<tr><td>2단계<br>(2027 상반기)<br>B2B 유료화</td><td>웹툰·자막 벤더·번역업체 대상 용어 검수 API·구독 서비스를 출시합니다. 1단계의 공공 도입 실적을 신뢰 근거로 영업하며, 예비창업패키지·글로벌 진출 지원사업과 연계합니다.</td></tr>
<tr><td>3단계<br>(2027 하반기~)<br>규모화</td><td>플랫폼·OTT 납품 벤더로 고객을 넓히고, 문화용어 데이터 라이선스와 재외문화원 네트워크를 통한 해외 파트너 진출을 추진합니다. (기창업자의 경우: 기존 콘텐츠·번역 사업에 본 엔진을 결합하여 저자원 언어권 수출을 확대합니다.)</td></tr>
</table>

<h3>나. 경쟁 우위와 진입장벽</h3>
<ul>
<li><b>확보하기 어려운 데이터 기반:</b> 공공데이터의 공인 표준과 한국국제교류재단의 39년 다국어 자료를 결합한 기반은 후발 주자가 단기간에 갖추기 어렵습니다.</li>
<li><b>무경쟁 포지션:</b> 범용 번역기는 '문화용어 표준 준수'를 목표로 삼지 않아, 이 축에서는 직접 경쟁자가 사실상 없습니다.</li>
<li><b>레퍼런스 장벽:</b> 공공 도입 실적 자체가 후발 진입을 어렵게 하는 신뢰 자산이 됩니다.</li>
</ul>

<h3>다. 현실적 한계와 대응 (정직하게)</h3>
<table>
<tr><th style="width:26%">한계</th><th>대응</th></tr>
<tr><td>범용 번역보다 작은 니치 시장</td><td>경쟁이 적고 진입장벽·마진이 높은 영역. 관광·교육·문화유산 등 인접 도메인으로 대상을 확장</td></tr>
<tr><td>공공(B2G) 조달의 긴 영업 사이클</td><td>파일럿·조달 등록을 병행하고, 본 경진대회 수상 실적을 초기 신뢰 근거로 활용</td></tr>
<tr><td>AI 운영 비용</td><td>반복 요청 재사용(캐싱)·배치 처리·경량 모델 혼용으로 원가 관리(설계 완료)</td></tr>
<tr><td>「Koreana」 원문 저작권</td><td>파생 자료만 활용, 원문 미재배포, 상업화 전 재단과 이용 협의</td></tr>
</table>
