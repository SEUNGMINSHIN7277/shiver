...
## 0.5 현재 최우선 목표 (S0 스프린트)

**7/13(월) 18:00 접수 마감 전까지 데모 v0 완성** — DEV_SPEC §12의 S0 계획을 따른다.
사용자 입력(키·데이터) 없이 진행 가능한 S0-A 태스크를 우선 소진하고, 사용자 산출물이
`data/pilot/` 또는 채팅으로 도착하면 즉시 S0-B 통합으로 전환한다. 완성도보다 **동작하는
수직 슬라이스**(용어사전→번역→화면)가 우선. 이 섹션은 접수 후 삭제한다.
