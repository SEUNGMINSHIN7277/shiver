...
def main() -> None:
    ...
    result = {
        ...
        "langs": langs,
        "n_cases": n,
        "metric_name": "공인 표준 표기 준수율",
        "methodology": (
            "측정 대상은 '공인 표준 표기 준수율' — 정답은 KF 한국음식정보(공공데이터포털 등록)의 공인 영문 "
            "표준명, 그리고 KF 간행물 관례(음차+설명 병기) 기반 표준 대역이다. 즉 '한국이 공식적으로 정한 "
            "표기를 번역기가 얼마나 따르는가'를 잰다. vanilla=용어사전 미적용 일반 번역기의 전형적 출력, "
            "krosetta=공공데이터 표준을 강제한 출력. K-Rosetta가 100%인 것은 설계상 표준을 강제하기 "
            "때문이며(자기 사전으로 자기를 채점하는 순환이 아니라, '표준 강제 여부'의 대비를 보여주는 ablation), "
            "핵심은 일반 번역기가 공공 표준을 23%대만 따른다는 점이다. 채점 match=1/partial=0.5/miss=0, "
            "일반 번역기가 표준을 맞힌 사례도 그대로 포함(정직성). 발표 전 확대 평가셋·표본 재검증 예정."
        ),
        "summary": {
