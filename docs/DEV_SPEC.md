...
### 7.0 ★ 비용 0원 모드 (현재 기본 — 사용자 결정 2026-07-11)

사용자는 Claude Max 구독만 보유(API 키 별도 과금). **현금 지출 없이** 개발을 진행한다:

- **오프라인 LLM 작업(용어 추출·역어 정렬·번역 생성·벤치마크 채점)은 API 호출 대신
  Claude Code 세션이 직접 수행**하고 결과를 데이터 파일(`data/pilot/derived/`,
  `backend/tests/fixtures/`)로 커밋한다. 구독 사용량만 소모, 현금 0원.
- **런타임은 `DEMO_MODE=cached`가 기본**: 번역·챗봇 API는 사전 계산된 결과 저장소
  (`data/demo_cache/`)에서 서빙. 데모 시나리오(발표용 3종 + 자유 입력 예시 N종)를
  세션에서 미리 생성해 채워 넣는다. 용어사전 검색·RAG 검색·공공데이터 연동은
  LLM 불필요라 원래부터 라이브로 동작(임베딩은 로컬 BGE-M3, 무료).
- **벤치마크 베이스라인도 0원으로**: ① "용어사전 없는 바닐라 번역 vs 용어사전 제약 번역"
  ablation(동일 모델, 정직하고 과학적인 비교 — 세션에서 생성), ② Google 번역 웹(무료)
  결과를 30~50문장 수동 확보(사용자 30분 or 세션 안내). "GPT 비교"는 OpenAI 키가
  생기는 경우에만 추가.
- **API 키 전환 시점(선택)**: 서류 통과 → 발표 준비 단계에서 자유 입력 라이브 데모를
  원할 때만 소액($10~20) 충전 결정. 코드는 `DEMO_MODE=live` 스위치 하나로 전환되도록
  이중 구조를 유지한다 (`services/llm/`이 cached/live 백엔드를 추상화).

### 7.1 라이브 모드 명세 (`DEMO_MODE=live` 전환 시)
...
- 라이브 전환 시 예상 비용: 데모 운용 수준 $10~20, 풀 파이프라인까지 API로 돌릴 경우 $50~120
  (Batches 50% 할인 기준). 지출 전 반드시 사용자 승인.
...
> **비용 0원 원칙(§7.0)에 따라 API 키류는 크리티컬 패스에서 제외됨. 아래는 전부 무료 액션.**

| 시점 | 사용자 액션 | 소요 | 없으면 막히는 것 |
|---|---|---|---|
| **오늘 밤 (7/11)** | ① data.go.kr 가입 → 파일데이터 2종 다운로드(한국음식정보 15044203, 디지털아카이브 기사목록 15139278) → 채팅 업로드 또는 `data/pilot/opendata/`에 커밋 | 20분 | 시드 글로서리, R1 요건 실데이터 |
| **오늘 밤** | ② 오픈API 활용신청 4종 (15075335, 15075346, 15052909, 15099215) — 무료, 심의승인이면 수일 걸리므로 지금 신청 | 15분 | 라이브 API 시연 (파일 2종이 있으면 R1 자체는 충족) |
| **내일 아침 (7/12)** | ③ 파일럿 크롤러 로컬 실행 (`python scripts/run_pilot_crawl.py`) → 산출물(`data/pilot/koreana/`) 커밋 또는 업로드 | 실행 30분~1h (방치 가능) | Koreana 기반 용어사전·RAG — **데모의 심장** |
| 내일 중 | ④ (선택, 무료) Google 번역 웹에서 벤치마크 문장 30~50개 번역 결과 확보 — 세션이 입력 문장 목록을 준비해 줌 | 30분 | 외부 MT 비교축 (없으면 ablation 비교만) |
| **7/13 오전** | ⑤ 기획서 초안 검토·수정 (Claude가 C3에서 작성) | 1h | — |
| **7/13 18:00 전** | ⑥ 접수 제출 (국민생각함 또는 opendata@kf.or.kr) — 참가신청서+기획서+개인정보동의서 | 30분 | **전부** |
| (선택, 추후) | Anthropic API 키 소액 충전 — 발표 단계에서 자유입력 라이브 데모를 원할 때만 (§7.0) | 10분 | 자유 입력 라이브 모드만 (캐시드 데모는 무관) |
