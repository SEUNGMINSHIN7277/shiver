...
- T1.4 ★ data.go.kr 데이터셋 연동 (R1 — **최우선**): 파일데이터 2종(한국음식정보 15044203,
  디지털아카이브 기사목록 15139278) 적재 → 글로서리 시드·RAG 소스로 결합, KOICA ODA 용어사전
  이식, 오픈API(표준국문명칭·국가표준코드) 클라이언트 + provenance 기록
- T1.5 수집 통계 리포트
